...
class AdvancedConfigManager:
    ...
    def __init__(self, config_dir: str = None):
        ...
        self._configs = {
            level: {scope: {} for scope in ConfigScope} for level in ConfigLevel
        }
    ...
    def _setup_default_schemas(self):
        ...
        basic_schemas = [
            ConfigSchema(
                "theme", str, "dark", False, "Tema de la interfaz", ConfigScope.UI
            ),
            ...
            ConfigSchema(
                "compact_mode", bool, False, False, "Modo compacto", ConfigScope.UI
            ),
            ConfigSchema(
                "animations_enabled", bool, True, False, "Animaciones", ConfigScope.UI
            ),
            ConfigSchema(
                "offline_mode", bool, True, False, "Modo offline", ConfigScope.PWA
            ),
            ConfigSchema(
    ...
    def list_schemas(self, scope: ConfigScope = None) -> List[ConfigSchema]:
        ...
        if scope:
            return [
                schema for schema in self._schemas.values() if schema.scope == scope
            ]
        else:
    ...
    def get_stats(self) -> Dict:
        ...
        configured_values = len(
            [
                k
                for k, v in self._compiled_config.items()
        ...
        return {
            ...
            "last_compile": (
                self._last_compile.isoformat() if self._last_compile else None
            ),
            "config_dir": self.config_dir,
            "scopes": {
                scope.value: len(
                    [s for s in self._schemas.values() if s.scope == scope]
                )
