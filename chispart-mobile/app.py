...
from config_extended import (
    ...
    AVAILABLE_APIS,
    DEFAULT_API,
    get_vision_supported_apis,
    get_pdf_supported_apis,
    validate_api_model_combination,
    get_api_statistics,
)
...
class ChispartMobileApp:
    ...
    def __init__(self):
        ...
        print(
            f"🚀 Chispart Mobile inicializado ({'Termux' if self.is_mobile else 'Desktop'})"
        )
    ...
    def _register_routes(self):
        ...
        def index():
            ...
            return render_template(
                "index.html", is_mobile=self.is_mobile, config=self._get_client_config()
            )
        ...
        def chat_page():
            """Página de chat"""
            return render_template(
                "chat.html", is_mobile=self.is_mobile, config=self._get_client_config()
            )
        ...
        def about_page():
            """Página 'Acerca de'"""
            return render_template(
                "about.html", is_mobile=self.is_mobile, config=self._get_client_config()
            )
        ...
        def api_chat():
            ...
            try:
                ...
                client = UniversalAPIClient(
                    api_key, self._get_api_base_url(api_name), api_name
                )
            ...
            if request.method == "GET":
                ...
                try:
                    ...
                    if success:
                        ...
                        return (
                            jsonify(
                                {"success": False, "error": "Error eliminando API key"}
                            ),
                            400,
                        )
        ...
        def validate_keys():
            ...
            try:
                ...
                if provider:
                    ...
                    result = asyncio.run(
                        self.api_manager.validate_api_key(provider, force_refresh=True)
                    )
        ...
        def api_history():
            ...
            try:
                ...
                return jsonify({"history": recent, "total": len(history)})
                ...
                return jsonify(
                    {
                        ...
                        "supported_features": {
                            ...
                            "total_models": sum(
                                len(get_available_models(api))
                                for api in AVAILABLE_APIS.keys()
                            ),
                        },
                    }
                )
        ...
        def api_interactive():
            ...
            if user_input.startswith("@analizar"):
                ...
                if len(parts) < 2:
                    return (
                        jsonify(
                            {"status": "error", "output": "Uso: @analizar <directorio>"}
                        ),
                        400,
                    )
                ...
                if result["status"] == "passthrough":
                    ...
                    result["output"] = (
                        f"Comando no reconocido o chat: {result['output']}"
                    )
        ...
        for provider_id, config in AVAILABLE_APIS.items():
            ...
            if provider_id in configured_names:
                configured_info = next(
                    p for p in configured_providers if p["provider"] == provider_id
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Chispart Mobile - Universal LLM Terminal"
    )
    parser.add_argument("--host", default="0.0.0.0", help="Host para el servidor")
    parser.add_argument(
        "--port", type=int, default=5000, help="Puerto para el servidor"
    )
