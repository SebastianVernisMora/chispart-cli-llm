...
class TestPerformance:
    def test_response_time(self, app_client):
        ...
        for endpoint in endpoints:
            ...
            assert (
                response_time < 1.0
            ), f"Endpoint {endpoint} demasiado lento: {response_time:.2f}s"

        # Imprimir resultados para referencia
        print("\nTiempos de respuesta:")
        for endpoint, data in results.items():
            print(
                f"  {endpoint}: {data['response_time']:.4f}s (HTTP {data['status_code']})"
            )
    ...
    async def test_api_response_time(self, app_client):
        ...
        for endpoint in endpoints:
            ...
            assert (
                response_time < 0.5
            ), f"API endpoint {endpoint} demasiado lento: {response_time:.2f}s"
